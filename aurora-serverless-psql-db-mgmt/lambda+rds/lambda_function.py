...
def connect_and_authenticate(secret_dict, port, dbname, use_ssl):
    ...
    try:
        ...
        logging.info("Successfully established %s connection as user '%s' with host: '%s'", 
        ...
        if "server does not support SSL, but SSL was required" in error_message:
            logging.error("Unable to establish SSL/TLS handshake, SSL/TLS is not enabled on the host: %s", secret_dict['host'])
        elif re.search(r'server common name ".+" does not match host name ".+"', error_message):
            logging.error("Hostname verification failed when establishing SSL/TLS Handshake with host: %s", secret_dict['host'])
        elif re.search(r'no pg_hba.conf entry for host ".+", SSL off', error_message):
            logging.error("Unable to establish SSL/TLS handshake, SSL/TLS is enforced on the host: %s", secret_dict['host'])
        return None
